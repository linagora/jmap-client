...
export default class Client {
  ...
  createMailbox(name, parentId) {
    ...
    return this.setMailboxes({
      ...
      return new CreateMailboxAck(this, response.created[clientId]);
    });
  }
   ...
   */
  send(message) {
    return this._createMessage(message, MailboxRole.OUTBOX);
  }
  ...
  _jmapRequest(command, options) {
    return this.transport
      ...
      .then(data => data.map(this._handleResponse.bind(this)))

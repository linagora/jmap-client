    ...
    value: function moveMessage(id, mailboxIds) {
      ...
      return this.setMessages({
        ...
        if (messagesSet.updated.indexOf(id) < 0) {
          throw new Error('Failed to move message ' + id + ', the reason is: ' + messagesSet.notUpdated[id]);
        }
      });
    }
...
var CreateMailboxAck = (function (_Model) {
  ...
  function CreateMailboxAck(jmap, response) {
    ...
    _utilsUtilsJs2['default'].assertRequiredParameterHasType(response.mustBeOnlyMailbox, 'response.mustBeOnlyMailbox', 'boolean');
